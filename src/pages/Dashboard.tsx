        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-3 md:mb-4 gap-2">
          <div>
            ...
            <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">
              {imagesLoading ? "Loading your image count..." : `You have generated ${images.length} image${images.length === 1 ? "" : "s"}.`}
            </p>
          </div>
        </div>

        <TabsContainer createContent={<CreateTabContent images={images} imagesLoading={imagesLoading} fetchError={fetchError} generateImage={generateImage} generating={generating} generationError={generationError} retryLastGeneration={retryLastGeneration} hasLastPrompt={hasLastPrompt} />} galleryContent={<GalleryTabContent images={images} loading={imagesLoading} error={fetchError} onRetry={fetchImages} />} />
      </div>
    </DashboardLayout>;
};
